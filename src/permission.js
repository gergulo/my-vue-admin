...
function hasPermission(userPerms, routerPerm) {
  // 特殊值，*代表所有资源权限
  if (userPerms.some(p => p.perm_value === '*')) return true
  // 如果菜单路由上没有声明perm属性，默认显示该菜单，代表所有人可以访问
  if (!routerPerm) return true
  // 判断用户的资源权限集合中是否包含该菜单路由声明的资源权限
  return userPerms.some(p => p.perm_value === routerPerm)
}
...
router.beforeEach((to, from, next) => {
  ...
  if (hasToken) {
    ...
    if (to.path === loginRoute) {
      // 1.1 如果是去登录页，有token视为已登录，直接跳到控制台
      next({ path: '/' })
      ...
      if (store.getters.visitor) {
        ...
        if (hasPermission(store.getters.perms, to.meta.permission)) {
          next()
        } else {
