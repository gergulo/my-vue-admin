...
export default{
  ...
  mapToMenuPermissionTree(routeArr, parentPermVal) {
    return routeArr.map(route => {
      const obj = {}
      if (route.meta && route.meta.perm) {
        obj.perm_value = route.meta.perm
        obj.perm_name = route.meta.title
        obj.perm_type = permType.MENU
        obj.parent = parentPermVal
      }
      if (route.children) {
        obj.children = this.mapToMenuPermissionTree(route.children, obj.perm_value)
      }
      return obj
    })
  },
  ...
  mapToButtonPermissionTree(btnPermMap, menuPermissionTree) {
    return menuPermissionTree.map(perm => {
      if (perm) {
        ...
        if (perm.perm_type === permType.MENU) {
          const btnPerms = btnPermMap[perm.perm_value]
          if (btnPerms) {
            btnPerms.forEach(p => {
